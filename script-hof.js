...
// Q1
let totalPop = data.reduce((acc, curr) => {
  // opt1
...
let totalInd = data.filter((item) => {
  if (item.independent) {
    return item;
  }
});

q2.innerHTML = totalInd.length;

// Q3

// let ccn3_275 = data.find((item) => {
//   if (item.ccn3 === "275") {
//     return item;
//   }
// });

let ccn3_275 = data.find((item) => item.ccn3 === "275");

q3.innerHTML = ccn3_275.population.toLocaleString();
...
// Q5
let countryIndex = data.findIndex((item) => {
  if (item.name.common === "Liberia") {
    return item;
  }
});
